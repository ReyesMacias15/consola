    ...
    do {
      ...
      switch (resmenu) {
        case "1": // agregar tarea
          ...
          //--------FUNCION PARA REGISTRAR CLIENTE/ADMINISTRADOR-----
          break;
        case "2": // ver tareas
          ...
          axios.post(
            "http://localhost:3000/v1/salaevento/api/cliente/",
            postData
          );
          axios
            .post(
              "http://localhost:3000/v1/salaevento/api/administradores/",
              postData
            )
    ...
    switch (restipo) {
      case "1": // Casa para iniciar sesion
        ...
        do {
          ...
          axios
            .get("http://localhost:3000/v1/salaevento/api/cliente/sesion", {
              params: {
                Usuario: restsesion.Usuario,
                Contrasena: restsesion.Contrasena,
              },
            })
            .then(async function (res) {
              usuarios = res.data;
        ...
        switch (restcliente) {
          case "1": // agregar tarea para tipo de sala para alquilar
            ...
            axios
              .get("http://localhost:3000/v1/salaevento/api/tipo_sala")
              .then(async function (res) {
                tipo_sala = res.data;
              })
              .catch(function (err) {
      ...
      case "2":
        do {
          //---------FUNCION PARA INICIO DE SESION EN ADMINISTRADOR-------
          restsesion = await Sesion();
          await pausa(console.log(restsesion));
          axios
            .get(
              "http://localhost:3000/v1/salaevento/api/administradores/sesion",
              {
                params: {
                  Usuario: restsesion.Usuario,
                  Contrasena: restsesion.Contrasena,
        ...
        switch (restadmin) {
          //-----FUNCION PARA AGREGAR LAS SALAS-----
          case "1": // agregar sala
            ...
            //Guardamos en la base de datos.
            axios
              .post("http://localhost:3000/v1/salaevento/api/salas/", postData)
              .then(async function (res) {
                await pausa(console.log(res.data));
              });
            break;
...
const buscarReserva = async (usuarios) => {
  const estadoConectado = await mongoose.connect(conexion);
...
const GuaradarSala = async (sala, usuario) => {
  const estadoConectado = await mongoose.connect(conexion);
...
const Actualizar_pago = async (reservaciones, reserva) => {
  const estadoConectado = await mongoose.connect(conexion);
  await pausa(console.log(reservaciones));
  ...
  //actualizar el monto
  const conwe = await Reservas.updateOne(id, { $set: datos });
  //actualizar tipo de pago
  const sala = await Reservas.updateOne(id, { $set: datos1 });
  await pausa(console.log(conwe));
  return conwe;
};
...
const salas_tipo = async (restipos) => {
  axios
    .get("http://localhost:3000/v1/salaevento/api/salas/id", {
      params: {
        _id: restipos,
      },
    })
...
const BuscarUsuario = async (restipos) => {
  //conexion
  const estadoConectado = await mongoose.connect(conexion);
  //estructurar para q mongo lo entienda
  const dato = {
    Usuario: `${restipos.Usuario}`,
    Contrasena: `${restipos.Contrasena}`,
  };
